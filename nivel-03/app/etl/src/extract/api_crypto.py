...
from src.core.load_staging import load_staging

def extract_crypto_to_staging():
    """
    Consome a API CoinGecko, salva o JSON bruto e carrega o Parquet no Postgres.
    """
    engine = get_engine()
    table_name = 'stg_crypto'
    ...
    try:
        ...
        # 1. SALVAR RAW
        file_path = os.path.join(RAW_DIR, 'crypto_data_raw.json')
        ...
        # 3. SALVAR STAGING
        parquet_path = os.path.join(STAGING_DIR, 'stg_crypto.parquet')
        df.to_parquet(parquet_path, index=False)
        print(f"✅ Parquet de cripto atualizado em: {parquet_path}")
        
        load_staging(df, table_name, engine)
